...
def process_dates(dates):
    ...
    try:
        get_closest_dates(dates)
    except Exception:
        print("get_closest_dates(dates) exception")

    date_list = ", ".join(d.get('date') for d in dates)
    print(date_list)


def schedule_if_possible(dates):
...
def should_rest(t0):
    ...
    print(f"\nWorking Time:  ~ {elapsed_time / 60:.2f} minutes")
    return elapsed_time > WORK_LIMIT_TIME * 3600


def rest_program():
    driver.get(SIGN_OUT_LINK)
    time.sleep(WORK_COOLDOWN_TIME * 3600)
